...
def log(message: str) -> None:
    print(f"[ops-reconcile] {message}", flush=True)
...
def ensure_ssh_keypair() -> None:
    if SSH_PRIVATE_KEY_PATH.exists() and SSH_PUBLIC_KEY_PATH.exists():
        ...
        SSH_PUBLIC_KEY_PATH.chmod(0o644)
        return
...
def publish_ssh_public_key(conn) -> None:
    public_key = SSH_PUBLIC_KEY_PATH.read_text(encoding="utf-8").strip()
    with conn.cursor() as cur:
        cur.execute(
            """
            INSERT INTO sensos.runtime_operator_keys (component, ssh_public_key, updated_at)
            VALUES (%s, %s, NOW())
...
def ensure_ops_peer(conn, network_id: int, ops_ip: str) -> int:
    with conn.cursor() as cur:
        cur.execute(
            """
            SELECT id
        ...
        cur.execute(
            ...
            RETURNING id;
            """,
            (network_id, ops_ip, "Ops Container"),
        )
        return cur.fetchone()[0]


def ensure_active_ops_key(conn, peer_id: int, public_key: str) -> None:
    with conn.cursor() as cur:
        cur.execute(
            """
...
def render_interface_config(
    ...
    return (
        ...
        f"Endpoint = {SERVER_COMPONENT}:{server_listen_port}\n"
...
def reconcile_network(
    ...
    ops_ip = str(ip_range.network_address + OPS_HOST_OFFSET)
    private_key_path = ensure_private_key(name)
    public_key = derive_public_key(private_key_path)
    peer_id = ensure_ops_peer(conn, network_id, ops_ip)
    ensure_active_ops_key(conn, peer_id, public_key)
